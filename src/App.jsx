...
const AssetIcons = {
  None: () => (
    <svg className="w-3.5 h-3.5 text-slate-800 dark:text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3">
      <circle cx="12" cy="12" r="10" strokeDasharray="3 3" />
    </svg>
  ),
  heavy: () => (
    <svg className="w-3.5 h-3.5 text-blue-800 dark:text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
  ...
  case: () => (
    <svg className="w-3.5 h-3.5 text-cyan-800 dark:text-cyan-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
      <rect x="3" y="7" width="18" height="12" rx="2" />
      <path strokeLinecap="round" strokeLinejoin="round" d="M8 7V5a2 2 0 012-2h4a2 2 0 012 2v2M12 11v4M9 13h6" />
    </svg>
  ),
  lightning: () => (
    <svg className="w-3.5 h-3.5 text-amber-800 dark:text-amber-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
      <path strokeLinecap="round" strokeLinejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" />
    </svg>
  ),
  tester: () => (
    <svg className="w-3.5 h-3.5 text-emerald-800 dark:text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
  ...
  crane: () => (
    <svg className="w-3.5 h-3.5 text-purple-800 dark:text-purple-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
...
export default function App() {
  ...
  const getRowBgColor = (task, index) => {
    ...
    if (isDarkMode) {
      ...
      if (isApproved) return 'bg-emerald-100 border-emerald-600 hover:bg-emerald-150';
      return index % 2 === 0 ? 'bg-white border-slate-400 hover:bg-slate-100' : 'bg-slate-50 border-slate-400 hover:bg-slate-100';
    }
  };

  const getBadgeStyle = (status) => {
    if (status === 'APPROVED') {
      return 'bg-emerald-600 text-white dark:bg-emerald-500/10 dark:text-emerald-400 border-emerald-700 dark:border-emerald-500/40';
    }
    if (status === 'HOLD') {
      return 'bg-rose-600 text-white dark:bg-rose-500/10 dark:text-rose-400 border-rose-700 dark:border-rose-500/40 animate-pulse font-black';
    }
    return 'bg-blue-600 text-white dark:bg-blue-500/10 dark:text-blue-400 border-blue-700 dark:border-blue-500/40';
  };
  ...
  return (
    <div className={`flex flex-col h-screen font-sans overflow-hidden transition-all duration-300 ${isDarkMode ? 'bg-[#080d1a] text-slate-100' : 'bg-[#f8fafc] text-slate-950'}`}>
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        * {
        ...
        /* Webkit and iOS Specific momentum scroll fixes */
        .overflow-y-auto, .overflow-x-auto {
          -webkit-overflow-scrolling: touch;
        }

        /* Prevent Safari iOS select menu defaults */
        select, input {
        ...
        .gantt-grid-light {
          background-image: 
            linear-gradient(to right, rgba(71, 85, 105, 0.35) 1px, transparent 1px),
      ...
      {}
      <header className={`border-b z-20 shrink-0 transition-colors ${isDarkMode ? 'bg-[#0f172a] border-slate-800' : 'bg-white border-slate-400'}`}>
        <div className="px-4 py-3 sm:px-6 sm:py-4 flex flex-col md:flex-row md:items-center justify-between gap-3 sm:gap-4">
          <div className="flex items-center gap-2 sm:gap-3 justify-between md:justify-start">
                ...
                <div className="relative flex items-center gap-2">
                  <button
                    ...
                    className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border transition-all text-xs font-black select-none ${
                      isDarkMode 
                        ...
                        : 'bg-slate-100 hover:bg-slate-200 text-slate-950 border-slate-500 hover:border-slate-700 shadow-sm'
                    }`}
                  >
                    ...
                    <ChevronDown className="text-slate-800 dark:text-slate-500 shrink-0" size={12}/>
                  </button>
                  ...
                  {isProjectDropdownOpen && (
                    <div className={`absolute left-0 top-10 w-72 rounded-2xl shadow-2xl border p-2 z-50 animate-in fade-in slide-in-from-top-1 duration-200 ${
                      isDarkMode ? 'bg-[#131c2e] border-slate-700 text-slate-200' : 'bg-white border-slate-400 text-slate-950'
                    }`}>
                      <div className="px-3 py-1.5 text-[9px] font-black tracking-widest text-slate-500 dark:text-slate-400 border-b border-slate-350 dark:border-slate-800 uppercase mb-2">
                        Schedules Directory
                      </div>
                          ...
                          <div 
                            ...
                            className={`flex items-center justify-between p-2 rounded-xl text-xs font-bold cursor-pointer transition-all ${
                              activeProjectId === proj.id 
                                ? 'bg-blue-600/15 text-blue-800 dark:text-blue-400 border border-blue-500 shadow-md' 
                                : (isDarkMode ? 'hover:bg-slate-800/80 text-slate-300 border border-transparent' : 'hover:bg-slate-100 text-slate-850 border border-transparent')
                            }`}
                          >
                            <div className="flex items-center gap-2 min-w-0">
                              <Folder className={activeProjectId === proj.id ? 'text-blue-500' : 'text-slate-500'} size={12}/>
                            ...
                            {proj.id !== 'master-schedule' && (
                              <button 
                                ...
                                className="p-1 text-slate-500 hover:text-rose-600 rounded transition-colors shrink-0"
                              >
                                <Trash2 size={12}/>
                              </button>
                            )}
                  ...
                  <span className="text-[10px] font-black px-2 py-0.5 rounded bg-blue-500/20 text-blue-800 dark:text-blue-400 shrink-0">
            ...
            {/* Theme Toggle Button for Quick Mobile Access */}
            <button 
              onClick={() => setIsDarkMode(!isDarkMode)} 
              className={`md:hidden p-2 rounded-xl border ${isDarkMode ? 'bg-slate-800 text-amber-400 border-slate-700' : 'bg-white text-slate-800 border-slate-400 shadow-sm'}`}
            >
          ...
          <div className="flex items-center justify-start overflow-x-auto scrollbar-none w-full md:w-auto -mx-4 px-4 md:mx-0 md:px-0">
            <div className={`flex items-center gap-1 border p-1 rounded-xl whitespace-nowrap ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-slate-200 border-slate-400'}`}>
              {[
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-[10px] sm:text-xs font-black uppercase tracking-wide transition-all ${
                    activePerspective === tab.id 
                      ? 'bg-blue-600 text-white shadow-md' 
                      : 'text-slate-850 dark:text-slate-400 hover:text-slate-950 dark:hover:text-slate-200'
                  }`}
                >
                  {tab.label}
                </button>
          ...
          <div className="flex gap-2 shrink-0 justify-end md:justify-start">
            <button 
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border shadow-sm transition-all text-xs font-black uppercase tracking-wider ${
                isControlPanelOpen
                  ...
                  : isDarkMode ? 'bg-[#131c2e] border-slate-700 text-slate-200 hover:bg-slate-800' : 'bg-white border-slate-400 text-slate-950 hover:bg-slate-100 shadow-sm'
              }`}
            ...
            <button 
              ...
              className={`p-2 rounded-xl transition border shadow-sm ${isDarkMode ? 'bg-[#131c2e] border-slate-700 text-slate-200 hover:bg-slate-800' : 'bg-white border-slate-400 text-slate-950 hover:bg-slate-100'} ${soundEnabled ? 'text-blue-600' : 'text-slate-550'}`} 
            >
              {soundEnabled ? <Volume2 size={14}/> : <VolumeX size={14}/>}
            </button>

            <button onClick={shareToGroupChat} className={`p-2 rounded-xl transition border shadow-sm ${isDarkMode ? 'bg-indigo-600/10 border-indigo-500/20 text-indigo-400' : 'bg-indigo-50 border-indigo-400 text-indigo-800'}`}>
              <MessageSquareShare size={14}/>
            </button>

            <button onClick={() => setIsNotificationPaneOpen(!isNotificationPaneOpen)} className={`p-2 rounded-xl transition border shadow-sm relative ${isDarkMode ? 'bg-[#131c2e] border-slate-700 text-blue-400 hover:bg-slate-800' : 'bg-white border-slate-400 text-blue-800 hover:bg-slate-100'}`}>
              <Bell size={14}/>
        ...
        {isControlPanelOpen && (
          <div className={`px-4 py-3 sm:px-6 sm:py-4 border-t flex flex-wrap gap-4 items-center animate-in slide-in-from-top-1 duration-200 ${isDarkMode ? 'bg-[#0b0f19]/90' : 'bg-slate-200/90 border-slate-400'}`}>
            <div className="flex flex-col gap-1">
              <span className="text-[10px] font-black text-slate-900 dark:text-slate-400 uppercase tracking-wider">Crew Structuring Profile</span>
              <div className={`flex border rounded-xl p-1 ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-slate-300 border-slate-400'}`}>
                <button 
                  onClick={() => { setLaborProfile('civil'); showToast("Switched to Civil Crew"); }}
                  className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase transition-all ${laborProfile === 'civil' ? 'bg-amber-600 text-white' : 'text-slate-850'}`}
                >
                  Civil
                </button>
                <button 
                  onClick={() => { setLaborProfile('electrical'); showToast("Switched to Electrical Crew ⚡"); }}
                  className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase transition-all ${laborProfile === 'electrical' ? 'bg-blue-600 text-white' : 'text-slate-850'}`}
                >
                  Electrical ⚡
                </button>
              </div>
            ...
            <div className="flex flex-col gap-1">
              <span className="text-[10px] font-black text-slate-900 dark:text-slate-400 uppercase tracking-wider">Stochastic Weather State</span>
              <div className="relative">
                <select 
                  ...
                  className={`text-xs px-3 py-2 border rounded-xl font-bold outline-none cursor-pointer pr-8 ${isDarkMode ? 'bg-[#131c2e] border-slate-700 text-blue-400 [&_option]:bg-[#131c2e]' : 'bg-white border-slate-400 text-slate-950'}`}
                >
            ...
            <div className="flex flex-col gap-1">
              <span className="text-[10px] font-black text-slate-900 dark:text-slate-400 uppercase tracking-wider">Base Launch Target</span>
              <div className={`flex items-center border rounded-xl px-3 py-1.5 text-xs font-bold ${isDarkMode ? 'bg-[#131c2e] border-slate-700 text-blue-500' : 'bg-white border-slate-400 text-slate-955'}`}>
                <input type="date" value={projectStartDate} onChange={(e) => setProjectStartDate(e.target.value)} className="outline-none bg-transparent cursor-pointer font-bold" />
              </div>
            </div>

            <div className="flex flex-col gap-1">
              <span className="text-[10px] font-black text-slate-900 dark:text-slate-400 uppercase tracking-wider">Document Specs</span>
              <button 
                onClick={() => setIsMetadataCollapsed(!isMetadataCollapsed)}
                className={`px-3 py-2 rounded-xl border text-xs font-black flex items-center gap-1.5 ${
                  isDarkMode ? 'bg-[#131c2e] border-slate-700 hover:bg-slate-800' : 'bg-white border-slate-400 hover:bg-slate-100'
                }`}
              >
            ...
            <div className="flex gap-2 ml-auto">
              <button onClick={() => setIsDarkMode(!isDarkMode)} className={`p-2 rounded-xl border ${isDarkMode ? 'bg-slate-850 text-amber-400 border-slate-700' : 'bg-white text-slate-800 border-slate-400 shadow-sm'}`}>
              ...
              <button onClick={() => setIsSettingsOpen(true)} className={`p-2 rounded-xl border ${isDarkMode ? 'bg-slate-850 border-slate-700' : 'bg-white border-slate-400'}`}>
                <Settings size={14}/>
              </button>
            </div>
          </div>
      ...
      {}
      <main className="flex-1 overflow-hidden p-3 sm:p-6 relative flex flex-col min-h-0">
        <div className="max-w-[1700px] w-full mx-auto flex flex-col gap-4 flex-1 min-h-0">
          ...
          {!isMetadataCollapsed && (
            <div id="gantt-export-zone" className={`rounded-2xl border p-4 flex flex-col gap-4 shadow-sm shrink-0 transition-all duration-300 animate-in slide-in-from-top-1 ${
              isDarkMode ? 'bg-[#131c2e] border-slate-700' : 'bg-white border-slate-400'
            }`}>
              <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div className="w-full md:w-1/2 text-center flex flex-col">
                  <input 
                    ...
                    className={`text-center font-black text-sm tracking-tight bg-transparent uppercase border-b border-transparent focus:border-blue-500 outline-none w-full transition-colors ${isDarkMode ? 'text-white' : 'text-slate-950'}`}
                    placeholder="Project Name..."
                  />
                  <input 
                    value={docMetadata.location} 
                    onChange={(e) => setDocMetadata({...docMetadata, location: e.target.value})}
                    className="text-center text-[10px] text-slate-800 dark:text-slate-400 font-bold uppercase tracking-wider bg-transparent border-none outline-none mt-1"
              ...
              <div className={`grid grid-cols-2 md:grid-cols-4 gap-4 text-[10px] font-black uppercase tracking-wider border-t pt-4 ${isDarkMode ? 'border-slate-800' : 'border-slate-400'}`}>
                <div className="flex flex-col gap-1 border-r border-slate-400 dark:border-slate-800">
                  <span>Document No:</span>
                  <input value={docMetadata.docNo} onChange={(e) => setDocMetadata({...docMetadata, docNo: e.target.value})} className={`font-mono bg-transparent outline-none w-full font-black ${isDarkMode ? 'text-slate-200' : 'text-slate-950'}`} />
                </div>
                <div className="flex flex-col gap-1 border-r border-slate-400 dark:border-slate-800 px-1">
                  <span>Revision:</span>
                  <input value={docMetadata.revision} onChange={(e) => setDocMetadata({...docMetadata, revision: e.target.value})} className={`bg-transparent outline-none w-full font-black ${isDarkMode ? 'text-slate-200' : 'text-slate-950'}`} />
                </div>
                <div className="flex flex-col gap-1 border-r border-slate-400 dark:border-slate-800 px-1">
                  <span>Effective Date:</span>
                  <input value={docMetadata.date} onChange={(e) => setDocMetadata({...docMetadata, date: e.target.value})} className={`bg-transparent outline-none w-full font-black ${isDarkMode ? 'text-slate-200' : 'text-slate-950'}`} />
                </div>
                <div className="flex flex-col gap-1 pl-1">
                  <span>Base Launch:</span>
                  <span className={`font-black ${isDarkMode ? 'text-slate-200' : 'text-slate-950'}`}>{projectStartDate}</span>
                </div>
              </div>
            </div>
          )}
          ...
          {activePerspective === 'gantt' && (
            <div className="flex-1 flex flex-col min-h-0 relative gap-3 animate-in fade-in duration-200">
              ...
              <div className={`p-3 rounded-2xl border flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 shrink-0 ${
                isDarkMode ? 'bg-[#131c2e]/60 border-slate-800' : 'bg-white border-slate-400 shadow-sm'
              }`}>
                <div className="flex items-center gap-3 flex-1">
                  <input 
                    ...
                    className={`px-4 py-2 rounded-xl text-xs font-bold border outline-none w-full max-w-sm transition-colors ${
                      isDarkMode 
                        ? 'bg-[#0f172a] border-slate-700 text-white focus:border-blue-500' 
                        : 'bg-slate-100 border-slate-450 text-slate-950 focus:border-blue-650'
                    }`}
                  />
                  
                  <button
                    onClick={() => setIsGanttSidebarVisible(!isGanttSidebarVisible)}
                    className={`hidden sm:flex items-center gap-1 px-3 py-2 rounded-xl text-xs font-black uppercase tracking-wider border transition-all ${
                      isGanttSidebarVisible 
                        ? 'bg-blue-600/10 border-blue-500/20 text-blue-800 dark:text-blue-400' 
                        : 'bg-slate-200 dark:bg-slate-800 border-slate-450 dark:border-slate-700 text-slate-850 dark:text-slate-400 hover:text-slate-950'
                    }`}
                  >
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all border ${
                        filterStatusTag === tag
                          ...
                          : (isDarkMode ? 'bg-[#0f172a] border-slate-700 text-slate-400 hover:text-white' : 'bg-white border-slate-400 text-slate-950 hover:bg-slate-100')
                      }`}
                    >
                      {tag}
                    </button>
              ...
              {}
              <div className={`flex border rounded-3xl overflow-hidden flex-grow shadow-sm transition-colors ${isDarkMode ? 'bg-[#131c2e]/10 border-slate-800' : 'bg-white border-slate-400'}`}>
                
                {isGanttSidebarVisible && (
                  <div className={`flex w-[60%] sm:w-[50%] md:w-[40%] lg:w-[45%] flex-col shrink-0 z-10 border-r relative transition-all duration-300 ${
                    isDarkMode ? 'bg-[#0f172a] border-slate-800' : 'bg-white border-slate-400'
                  }`}>
                    ...
                    <div className={`h-[48px] min-h-[48px] max-h-[48px] p-2 font-black text-[9px] flex items-center uppercase tracking-widest border-b transition-colors ${
                      isDarkMode ? 'bg-slate-950/60 border-slate-850 text-slate-400' : 'bg-slate-200 border-slate-400 text-slate-950'
                    }`}>
                      ...
                      <span className="hidden sm:inline-block w-20 text-center shrink-0 font-black">Asset</span>
                      <span className="w-10 text-center shrink-0 font-black">Days</span>
                      <span className="hidden sm:inline-block w-12 text-center shrink-0 font-black">Done</span>
                    </div>
                    ...
                    >
                      {filteredTasks.map((task, index) => (
                        ...
                        >
                          <div className="w-8 text-center flex items-center justify-center shrink-0 font-black font-mono text-[10px] text-blue-700 dark:text-blue-500">
                            {task.ref}
                          </div>
                          
                          <div className={`flex-grow h-full flex flex-col justify-center px-2 border-l min-w-0 ${isDarkMode ? 'border-slate-800/40' : 'border-slate-400'}`}>
                            ...
                            <span className={`font-bold text-xs truncate leading-tight mt-0.5 ${isDarkMode ? 'text-slate-100' : 'text-slate-955'}`}>
                              {task.task}
                            </span>
                          </div>

                          <div className={`hidden sm:flex w-20 h-full border-l items-center justify-center px-1 shrink-0 ${isDarkMode ? 'border-slate-800/40' : 'border-slate-400'}`}>
                            <span className="text-[10px] font-black text-blue-800 dark:text-blue-400 bg-blue-500/20 px-1.5 py-0.5 rounded">
                              {task.assignedAsset !== 'None' ? task.assignedAsset : '-'}
                            </span>
                          </div>
                          
                          <div className={`w-10 h-full border-l flex items-center justify-center font-black text-center shrink-0 font-mono ${isDarkMode ? 'border-slate-800/40' : 'border-slate-400'}`}>
                            {task.duration}d
                          </div>

                          <div className={`hidden sm:flex w-12 h-full border-l flex-col items-center justify-center px-1 shrink-0 ${isDarkMode ? 'border-slate-800/40' : 'border-slate-400'}`}>
                            <span className="font-mono text-[9px] font-black text-slate-850 dark:text-slate-300">{task.progress || 0}%</span>
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="h-[64px] p-2 flex items-center border-t border-slate-300 dark:border-slate-800 sticky bottom-0 z-20 shrink-0">
                       <button onClick={addTask} className="text-[9px] font-black uppercase tracking-widest flex items-center gap-1 transition-all px-2 py-2.5 rounded-xl border border-dashed border-slate-550 dark:border-slate-700 w-full justify-center text-blue-700 dark:text-blue-400 hover:text-blue-800">
                ...
                <div className="flex-1 flex flex-col relative overflow-x-auto scrollbar-thin">
                  ...
                  <div className={`h-[48px] min-h-[48px] max-h-[48px] flex min-w-max sticky top-0 z-25 border-b transition-colors ${
                    isDarkMode ? 'bg-slate-900 border-slate-850' : 'bg-slate-200 border-slate-400'
                  }`}
                      ...
                      return (
                        <div key={day} className={`w-[44px] h-full flex-shrink-0 text-center border-r flex flex-col justify-center transition-colors ${
                          isDarkMode 
                            ...
                            : `border-slate-350 ${isSatSun ? 'bg-slate-300' : ''}`
                        }`}>
                          <span className="text-[9px] font-black leading-tight text-slate-950 dark:text-slate-400">{generateDateHeaderStr(projectStartDate, day).split(' ')[0]}</span>
                          <span className="text-[8px] font-bold text-slate-850 dark:text-slate-500 leading-tight uppercase">{generateDateHeaderStr(projectStartDate, day).split(' ')[1]}</span>
                        </div>
                      )
                    })}
                  </div>
                  ...
                  >
                    {filteredTasks.map((task) => {
                      ...
                      return (
                        <div 
                          key={task.id} 
                          className={`h-[54px] min-h-[54px] max-h-[54px] border-b relative flex items-center ${isDarkMode ? 'border-slate-800/40' : 'border-slate-400'}`}
                          style={{ width: `${headerDays.length * 44}px` }}
                        >
                          <div 
                            onClick={() => setActiveTaskModal(task.id)}
                            className={`absolute h-[24px] rounded-lg shadow-sm transition-all flex items-center justify-between overflow-hidden text-xs font-black border cursor-pointer hover:scale-[1.015] z-10 ${
                              conflict 
                                ? 'bg-amber-600/30 border-amber-600 text-amber-950 dark:text-amber-300 animate-pulse'
                                : task.qaStatus === 'HOLD' 
                                  ? 'bg-rose-600/30 border-rose-600 text-rose-950 dark:text-rose-300' 
                                  : task.qaStatus === 'APPROVED' 
                                    ? 'bg-emerald-600/30 border-emerald-600 text-emerald-950 dark:text-emerald-300' 
                                    : 'bg-blue-600/30 border-blue-600 text-blue-950 dark:text-blue-300'
                            }`}
                            style={{ left: `${(task.startDays) * 44 + 2}px`, width: `${(task.adjustedDuration * 44) - 4}px` }}
                          >
                            <div className="absolute top-0 left-0 h-full bg-blue-600/20" style={{ width: `${task.progress || 0}%` }} />
                            <span className="truncate px-2 font-black text-[9px] relative z-10 flex items-center gap-1.5">
                              <span>{task.ref}</span>
                              <span className="opacity-90">{task.task}</span>
                            </span>
                          </div>
                        </div>
                      )
                  ...
                  <div className={`h-[120px] border-t flex min-w-max items-end relative sticky bottom-0 z-25 transition-colors ${
                    isDarkMode ? 'bg-[#0b0f19] border-slate-850' : 'bg-slate-200 border-slate-450'
                  }`}
                  style={{ width: `${headerDays.length * 44}px` }}>
                    {headerDays.map(day => {
                      ...
                      return (
                        <div key={`hist-${day}`} className={`w-[44px] h-full flex-shrink-0 border-r flex flex-col justify-end items-center pb-4 ${isDarkMode ? 'border-slate-800/40' : 'border-slate-350'}`}>
                          {dayManpower > 0 && (
                            <div 
                              className={`w-4 rounded-t-md transition-all ${dayManpower > 12 ? 'bg-rose-600' : 'bg-blue-600'}`}
                              style={{ height: `${Math.max(4, heightPercentage)}%` }}
                            />
                          )}
                          <span className={`text-[9px] font-black font-mono mt-1 ${dayManpower > 12 ? 'text-rose-750 dark:text-rose-400' : 'text-slate-850 dark:text-slate-400'}`}>
          ...
          {/* PERSPECTIVE VIEW 2: 7-DAYS OUTLOOK */}
          {activePerspective === 'weekly' && (
            <div className="flex-1 flex flex-col gap-4 overflow-hidden min-h-0 animate-in fade-in duration-200">
              <div className={`p-6 rounded-3xl border flex flex-col sm:flex-row items-center justify-between gap-4 shrink-0 ${
                isDarkMode ? 'bg-[#131c2e] border-slate-800' : 'bg-white border-slate-400'
              }`}>
                <div>
                  ...
                  <p className="text-xs text-slate-850 dark:text-slate-400 mt-1">Aggregates ongoing operational sequences across all active databases matching today's timeline.</p>
                </div>
                
                <div className={`flex items-center gap-1 border p-1 rounded-xl ${isDarkMode ? 'bg-slate-950/40 border-slate-850' : 'bg-slate-200 border-slate-450'}`}>
                  <button
                    onClick={() => setActiveForecastFilter('this-week')}
                    className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all ${
                      activeForecastFilter === 'this-week' ? 'bg-blue-600 text-white' : 'text-slate-850 dark:text-slate-400 hover:text-slate-950'
                    }`}
                  >
                    📅 This Week
                  </button>
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all ${
                      activeForecastFilter === 'all-active' ? 'bg-blue-600 text-white' : 'text-slate-850 dark:text-slate-400 hover:text-slate-950'
                    }`}
                  >
              ...
              {thisWeeksForecastList.length === 0 ? (
                <div className="flex-grow flex flex-col items-center justify-center p-8 rounded-3xl border border-dashed border-slate-400 dark:border-slate-800 text-center">
                  ...
                  <h4 className="text-xs font-black uppercase text-slate-850 dark:text-slate-400">No Operations Scheduled Inside this Forecast cycle</h4>
                  <p className="text-xs text-slate-800 dark:text-slate-550 max-w-sm mt-1">Try adding task rows, adjusting base dates, or shifting the timeline offsets.</p>
                </div>
              ) : (
                      ...
                      return (
                        <div
                          ...
                          className={`p-6 rounded-3xl border shadow-sm cursor-pointer transition-all hover:scale-[1.01] flex flex-col justify-between gap-4 ${
                            isDarkMode ? 'bg-[#131c2e] border-slate-850 hover:border-slate-700' : 'bg-white border-slate-400 hover:border-slate-600 shadow-md'
                          }`}
                        >
                          <div>
                            <div className={`flex justify-between items-start gap-2 border-b pb-3 ${isDarkMode ? 'border-slate-800' : 'border-slate-350'}`}>
                              <div className="min-w-0 flex-1">
                                <span className="bg-blue-600/15 text-blue-800 dark:text-blue-400 px-2.5 py-0.5 rounded text-[8px] font-black tracking-widest uppercase block truncate">
                                  {item.parentProjectName}
                                </span>
                                <h4 className={`text-sm font-black truncate mt-2 ${isDarkMode ? 'text-white' : 'text-slate-950'}`}>
                                  {item.task}
                                </h4>
                              </div>
                            ...
                            <div className="space-y-2 mt-4 text-xs font-bold text-slate-850 dark:text-slate-400">
                              ...
                              <div className="flex justify-between items-center text-[10px]">
                                ...
                                <span className="font-black text-blue-750 dark:text-blue-400">{item.assignedAsset || 'None'}</span>
                              </div>
                              <div className="flex justify-between items-center text-[10px]">
                                <span className="flex items-center gap-1 uppercase tracking-wide"><Users size={11}/> Workforce Loading</span>
                                <span className="font-black text-slate-900 dark:text-slate-300">{item.totalManpower || 0} Workers</span>
                              </div>
                            </div>
                          </div>

                          <div className={`space-y-1.5 pt-4 border-t ${isDarkMode ? 'border-slate-850' : 'border-slate-350'}`}>
                            <div className="flex justify-between text-[9px] font-black tracking-widest text-slate-800 dark:text-slate-400">
                          ...
                          {isOverdue && (
                            <div className="mt-1 p-2 bg-rose-500/15 border border-rose-650 rounded-xl text-rose-800 dark:text-rose-400 text-[10px] font-black flex items-center gap-1.5 animate-pulse">
          ...
          {activePerspective === 'qa' && (
            <div className="flex-grow flex flex-col gap-4 overflow-hidden min-h-0 animate-in fade-in duration-200">
              <div className={`p-6 rounded-3xl border flex items-center justify-between ${
                ...
                <div>
                  <h3 className="text-sm font-black tracking-widest uppercase text-emerald-650 dark:text-emerald-400 flex items-center gap-1.5">
                    <ShieldCheck size={16}/> Engineering Compliance Inspection Ledger
                  </h3>
                  <p className="text-xs text-slate-850 dark:text-slate-400 mt-1">Audit-ready verification list of all critical sequence hold points and requirements.</p>
                </div>
              </div>
                ...
                {activeFlowTasks.map(task => (
                  <div 
                    ...
                    className={`p-4 rounded-2xl border flex items-center justify-between cursor-pointer transition-all ${
                      isDarkMode ? 'bg-[#131c2e]/60 border-slate-800 text-slate-250' : 'bg-white border-slate-450 hover:border-slate-700 text-slate-950 shadow-sm'
                    }`}
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <span className="font-black font-mono text-xs text-blue-600 dark:text-blue-500">{task.ref}</span>
                      <div className="min-w-0">
                        <span className="text-[9px] uppercase tracking-wider font-black text-slate-800 dark:text-slate-500 block">{task.desc}</span>
                        <span className={`font-black text-xs block ${isDarkMode ? 'text-white' : 'text-slate-955'}`}>{task.task}</span>
                      </div>
                    </div>
                    
                    <div className="flex items-center gap-3">
                      <span className="text-xs font-black text-slate-850 dark:text-slate-400 hidden sm:inline">{Object.keys(task.checklist || {}).length} Parameters</span>
          ...
          {activePerspective === 'logistics' && (
            <div className="flex-grow flex flex-col gap-4 overflow-hidden min-h-0 animate-in fade-in duration-200">
              <div className={`p-6 rounded-3xl border flex items-center justify-between ${
                ...
                <div>
                  <h3 className="text-sm font-black tracking-widest uppercase text-purple-800 dark:text-purple-400 flex items-center gap-1.5">
                    <Truck size={16}/> Heavy Rig & Logistics Equipment Pool
                  </h3>
                  <p className="text-xs text-slate-850 dark:text-slate-400 mt-1">Configure site machinery, mobilization buffers, and check overlapping booking clashes.</p>
                </div>
                <button
                  ...
                  {customAssets.map(asset => {
                    ...
                    return (
                      <div key={asset.key} className={`p-5 rounded-3xl border flex flex-col justify-between gap-4 ${
                        isDarkMode ? 'bg-[#131c2e]/60 border-slate-850' : 'bg-white border-slate-400 shadow-sm'
                      }`}>
                        <div className="flex items-center gap-3 justify-between">
                          <div className="flex items-center gap-3 min-w-0">
                            <div className="p-2.5 rounded-xl bg-slate-200 dark:bg-slate-900 border border-slate-400 dark:border-slate-800 shrink-0">
                          ...
                          <button
                            ...
                            className="p-1 text-slate-650 hover:text-rose-600 hover:bg-rose-500/10 rounded-lg shrink-0 transition-all"
                          >
                            <Trash2 size={13}/>
                          </button>
                        </div>

                        <div className={`space-y-1.5 pt-3 border-t text-[10px] text-slate-850 dark:text-slate-400 ${isDarkMode ? 'border-slate-800' : 'border-slate-350'}`}>
                          <div className="flex justify-between font-bold">
                            <span>CLASS:</span>
                            <span className="font-black text-slate-950 dark:text-slate-300">{asset.type}</span>
                          </div>
                          <div className="flex justify-between font-bold">
                            <span>TRANSIT BUFFER:</span>
                            <span className="font-black text-slate-950 dark:text-slate-300">{asset.defaultMobilizationDays} Days</span>
                          </div>
                          <div className="flex justify-between font-bold">
                            <span>ACTIVE DISPATCHES:</span>
                            <span className="font-black text-blue-800 dark:text-blue-400">{isAllocated.length} Sequences</span>
                          </div>
                        </div>
                      </div>
                    )
          ...
          {tomorrowAllocations.length > 0 && (
            <div className={`p-3.5 rounded-2xl border flex flex-col sm:flex-row items-center justify-between gap-3 shrink-0 ${isDarkMode ? 'border-amber-500/30 bg-amber-500/10' : 'border-amber-650 bg-amber-200'}`}>
              <div className="flex items-center gap-3">
                <Clock className="text-amber-800 dark:text-amber-500 shrink-0" size={18}/>
                <p className="text-xs text-slate-955 dark:text-slate-300 font-bold text-left">
                  Tomorrow's logistical dispatch warnings detected across active schedule plans.
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                {tomorrowAllocations.map(task => (
                  <span key={task.id} className="bg-slate-900 border border-amber-600 px-3 py-1 rounded-lg text-[10px] font-black text-amber-400 flex items-center gap-1">
      ...
      {isAssetModalOpen && (
        <div className="fixed inset-0 bg-slate-950/85 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className={`rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden border flex flex-col max-h-[85vh] transition-all duration-300 ${
            isDarkMode ? 'bg-[#131c2e] border-slate-700 text-white' : 'bg-white border-slate-450 text-slate-950'
          }`}>
            <div className={`p-5 border-b flex justify-between items-center ${isDarkMode ? 'border-slate-850 bg-slate-950/40' : 'border-slate-400 bg-slate-100'}`}>
              <div>
            ...
            <div className="p-5 overflow-y-auto space-y-6 scrollbar-thin">
              <div className={`p-4 rounded-2xl border ${isDarkMode ? 'bg-slate-950/40 border-slate-850' : 'bg-slate-50 border-slate-400'} space-y-4`}>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[9px] font-black text-slate-850 dark:text-slate-400 uppercase mb-1">Asset Code (Unique Key)</label>
                    <input 
                      type="text" 
                      placeholder="e.g. CRN50, HVBT1"
                      value={newAssetCode}
                      onChange={(e) => setNewAssetCode(e.target.value)}
                      className={`w-full px-3 py-2 border rounded-xl text-xs font-bold outline-none ${isDarkMode ? 'bg-slate-900 border-slate-700 text-white' : 'bg-white border-slate-450 text-slate-950'}`}
                    />
                  </div>
                  <div>
                    <label className="block text-[9px] font-black text-slate-850 dark:text-slate-400 uppercase mb-1">Asset Label Name</label>
                    <input 
                      type="text" 
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    <label className="block text-[9px] font-black text-slate-850 dark:text-slate-400 uppercase mb-1">Class</label>
                    <div className="relative">
                      <select 
                        value={newAssetType}
                        onChange={(e) => setNewAssetType(e.target.value)}
                        className={`w-full px-3 py-2 border rounded-xl text-xs font-bold outline-none cursor-pointer pr-8 ${
                          isDarkMode ? 'bg-[#131c2e] text-white border-slate-700 [&_option]:bg-[#131c2e]' : 'bg-white text-slate-950 border-slate-450 [&_option]:bg-white'
                        }`}
                      >
                  ...
                  <div>
                    <label className="block text-[9px] font-black text-slate-850 dark:text-slate-400 uppercase mb-1">Preset Design</label>
                    <div className="relative">
                      <select 
                  ...
                  <div>
                    <label className="block text-[9px] font-black text-slate-850 dark:text-slate-400 uppercase mb-1">Mob Buffer</label>
                    <input 
                      type="number" min="0" max="7"
                      value={newAssetMobDays}
                      onChange={(e) => setNewAssetMobDays(parseInt(e.target.value) || 0)}
                      className={`w-full px-3 py-2 border rounded-xl text-xs font-bold outline-none ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-450'}`}
                    />
                  </div>
                </div>

                {assetValidationWarning && (
                  <div className="p-2.5 rounded-xl bg-rose-500/20 border border-rose-600 text-rose-850 dark:text-rose-400 text-[10px] font-black">
                    {assetValidationWarning}
                  </div>
                )}
        ...
        return (
          <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className={`rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden border flex flex-col max-h-[90vh] transition-all duration-300 ${
              ...
              <div className={`p-5 border-b flex justify-between items-center ${isDarkMode ? 'border-slate-850 bg-slate-950/40' : 'border-slate-400 bg-slate-100'}`}>
                <div>
                  <h3 className="text-xs font-black uppercase tracking-widest flex items-center gap-2"><ClipboardCheck className="text-blue-600" size={14}/> Site Field Inspector Card</h3>
                  <p className="text-[10px] text-slate-850 dark:text-slate-400 mt-1"><span className="text-blue-700 bg-blue-500/20 border border-blue-600 px-1.5 py-0.5 rounded mr-1.5 font-black">{currentTaskEditing.ref}</span> {currentTaskEditing.task}</p>
                </div>
                <button onClick={() => setActiveTaskModal(null)} className="p-1 rounded-lg hover:bg-slate-300"><X size={14}/></button>
              </div>
              
              <div className="p-6 overflow-y-auto flex-grow space-y-5 scrollbar-thin">
                <div className={`p-4 rounded-2xl border ${isDarkMode ? 'bg-slate-950/40 border-slate-850' : 'bg-slate-100 border-slate-400'}`}>
                  <h4 className="text-[9px] font-black uppercase tracking-wider text-slate-950 dark:text-slate-450 mb-2">Configure Base Days & Percent Complete</h4>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="flex flex-col gap-1">
                      <span className="text-[9px] font-black text-slate-850 dark:text-slate-500">DURATION (BASE DAYS)</span>
                      <div className="flex items-center gap-1.5 mt-1">
                        <button 
                          ...
                          className={`w-8 h-8 rounded-xl border flex items-center justify-center font-black text-sm ${isDarkMode ? 'bg-slate-900 border-slate-700 text-slate-200' : 'bg-white border-slate-400 text-slate-950'}`}
                        >
                          -
                        </button>
                        <span className="font-mono text-xs font-black text-blue-700 dark:text-blue-500 px-2">{currentTaskEditing.duration} Days</span>
                        <button 
                          onClick={() => {
                    ...
                    <div className="flex flex-col gap-1">
                      <span className="text-[9px] font-black text-slate-850 dark:text-slate-500">PERCENT COMPLETE (%)</span>
                      <input 
                        ...
                        className="mt-3.5 w-full cursor-pointer accent-blue-600 text-blue-600 bg-slate-300 dark:bg-slate-800"
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <h4 className="text-[9px] font-black uppercase tracking-wider text-slate-850 dark:text-slate-455">Co-Sign Inspector Status</h4>
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      { status: 'PENDING', label: 'Active Work', desc: 'In development', border: 'border-blue-600 text-blue-800 dark:text-blue-400', bg: 'bg-blue-600/20' },
                      { status: 'HOLD', label: 'QA HOLD', desc: 'Blocked point', border: 'border-rose-600 text-rose-800 dark:text-rose-400', bg: 'bg-rose-600/20' },
                      { status: 'APPROVED', label: 'APPROVED', desc: 'Inspection approved', border: 'border-emerald-600 text-emerald-800 dark:text-emerald-400', bg: 'bg-emerald-600/20' }
                    ].map((card) => (
                      <div
                        key={card.status}
                        onClick={() => {
                        ...
                        className={`p-3 rounded-2xl border text-center cursor-pointer transition-all ${
                          currentTaskEditing.qaStatus === card.status
                            ? `ring-2 ring-blue-600 ${card.border} ${card.bg}`
                            : (isDarkMode ? 'border-slate-850 bg-slate-900 text-slate-400' : 'border-slate-400 bg-slate-100 text-slate-850')
                        }`}
                      >
                        <span className="block font-black text-[10px] uppercase tracking-wide">{card.label}</span>
                ...
                <div className="space-y-2">
                  <h4 className="text-[9px] font-black uppercase tracking-wider text-slate-850 dark:text-slate-455">Shared Asset Allocation</h4>
                  <div className={`p-4 rounded-2xl border ${isDarkMode ? 'bg-slate-950/40 border-slate-850' : 'bg-slate-100 border-slate-400'} flex flex-col sm:flex-row items-center justify-between gap-3`}>
                    <div className="flex items-center gap-2">
                      <Truck className="text-blue-600" size={16}/>
                      <div className="text-left">
                        <p className="text-xs font-black">Assign Equipment/Specialized Tooling</p>
                      </div>
                    </div>
                    <div className="relative min-w-[200px]">
                      <select 
                        ...
                        className={`pl-3 pr-8 py-1.5 rounded-xl border text-xs font-black outline-none cursor-pointer w-full ${
                          isDarkMode ? 'bg-[#131c2e] text-white border-slate-700 [&_option]:bg-[#131c2e]' : 'bg-white text-slate-950 border-slate-450 [&_option]:bg-white'
                        }`}
                      >
                ...
                <div className={`p-4 rounded-2xl border ${isDarkMode ? 'bg-slate-950/40 border-slate-850' : 'bg-slate-100 border-slate-400'}`}>
                  <h4 className="text-[9px] font-black uppercase tracking-wider text-slate-850 dark:text-slate-455 mb-2 flex items-center gap-1.5"><ListTodo className="text-blue-600" size={12}/> Engineering Checklist</h4>
                  {Object.keys(currentTaskEditing.checklist || {}).length === 0 ? (
                    <p className="text-xs text-slate-850 dark:text-slate-500 italic font-bold">No checklist parameters set.</p>
                  ) : (
                      ...
                      {Object.entries(currentTaskEditing.checklist).map(([name, completed]) => (
                        <label 
                          ...
                          className={`flex items-center justify-between p-2.5 rounded-xl border text-xs cursor-pointer transition-all ${
                            completed 
                              ? 'bg-emerald-600/25 border-emerald-600 text-emerald-800 dark:text-emerald-400 font-black' 
                              : (isDarkMode ? 'bg-[#131c2e] border-slate-800 text-slate-200' : 'bg-white border-slate-400 text-slate-950')
                          }`}
                        >
                          <span>{name}</span>
                          <input 
      ...
      {isSettingsOpen && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className={`rounded-3xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col ${isDarkMode ? 'bg-[#131c2e] text-slate-100 border-slate-700' : 'bg-white text-slate-950 border-slate-450'}`}>
            <div className={`p-5 border-b flex justify-between items-center ${isDarkMode ? 'bg-slate-950/40 border-slate-850' : 'bg-slate-100 border-slate-400'}`}>
              <span className="font-bold flex items-center gap-2"><Settings size={14}/> Settings</span>
              <button onClick={() => setIsSettingsOpen(false)} className="p-1 rounded-lg hover:bg-slate-300"><X size={14}/></button>
            </div>
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-850 dark:text-slate-400 mb-2 uppercase">Left Contractor Logo</label>
      ...
      {isNotificationPaneOpen && (
        <div className="fixed inset-0 bg-slate-950/50 backdrop-blur-sm z-50 flex justify-end">
          <div className={`w-full max-w-sm h-full p-6 flex flex-col justify-between border-l shadow-2xl transition-all duration-300 ${isDarkMode ? 'bg-[#131c2e] border-slate-850 text-slate-100' : 'bg-white text-slate-450 text-slate-950'}`}>
            <div className="space-y-4 overflow-y-auto">
              <div className="flex justify-between items-center border-b pb-3 border-slate-300 dark:border-slate-800">
                <div className="flex items-center gap-2">
                  <Bell className="text-blue-600" size={14}/>
                  <h3 className="text-xs font-black uppercase tracking-wider">Site Activity Stream</h3>
                </div>
                <button onClick={() => setIsNotificationPaneOpen(false)} className="p-1 rounded-md hover:bg-slate-300">
                  <X size={14}/>
                </button>
              </div>
              ...
              <div className="space-y-2">
                {notifications.map((notif) => (
                  <div key={notif.id} className={`p-3 rounded-xl border text-xs font-bold ${isDarkMode ? 'bg-slate-900 border-slate-850 text-slate-300' : 'bg-slate-100 border-slate-400 text-slate-950'}`}>
      ...
      {showCreateProjectModal && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className={`rounded-3xl shadow-2xl w-full max-w-md overflow-hidden border ${isDarkMode ? 'bg-[#131c2e] border-slate-700' : 'bg-white border-slate-450'}`}>
            <div className="p-6">
              ...
              <input 
                ...
                className={`w-full px-4 py-3 rounded-xl border text-xs font-bold outline-none mb-4 ${
                  isDarkMode ? 'bg-slate-900 border-slate-700 text-white' : 'bg-slate-100 border-slate-450 text-slate-950'
                }`}
              />
              <div className="flex justify-end gap-2">
                <button onClick={() => setShowCreateProjectModal(false)} className="px-4 py-2 text-slate-600 font-bold text-xs">Cancel</button>
